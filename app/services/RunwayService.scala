...
object RunwayService {
  ...
  def getAirportAndRunwaysForCountry(countryNameOrCode: String): Array[Row] = {
    getAirportAndRunwaysByCountryCode(CountryService.getCountryCode(countryNameOrCode))
  }

  def getAirportAndRunwaysByCountryCode(countryCode: String): Array[Row] = countryCode match {
    case "" => Array()
    case _ =>
      createAllSchemas()
      sqlContext
